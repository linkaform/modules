...
#---Format Calc
def get_time_format(date_start, date_end):
...
#---OPtions
def set_data_update(curp_record, list_Data):
    #---Format Item
    dic_items = []
    if list_Data:
        for x in list_Data:
...
def get_data_query_items(curp_record):
    #---Query
    match_query = {
        "form_id":114233,
        "answers.5ea0897550b8dfe1f4d83a9f":curp_record,
...
def set_out_user(curp_record, location):
    #-----Query Information
...
#---New Functions
def get_data_query(location):
    #---Query
    match_query = {
        "form_id":107815,
        "answers.64ee8349998263ad9bec20cd":location,
        "answers.65cbe03c6c78b071a59f481c":{"$exists": False } ,
    }
        ...
        {"$project":
            {
                "folio":"$folio",
                "id":"$id",
                "curp": "$answers.64ee8349998263ad9bec20cc",
...
if __name__ == "__main__":
    ...
    if option !='':
        if option == 'query':
            lis_data = get_data_query(location)
