...
from payrus_settings import *
from generate_chart import Lkf_Chart

def set_status_proceso(current_record, record_id, new_estatus, msg=''):
    current_record['answers'].update({ 'a00000000000000000000003': new_estatus, 'a00000000000000000000004': msg })
...
"""
def get_record_producto_catalogo( catalog_id, field_id_catalogo, value_field_catalog ):
    if type(value_field_catalog) == list and value_field_catalog:
        value_field_catalog = value_field_catalog[0]
    mango_query = {"selector":
        {"answers":
            {"$and":[
                {field_id_catalogo: {'$eq': value_field_catalog}}
            ]}},
        "limit":1,
        "skip":0}
    res = lkf_api.search_catalog( catalog_id, mango_query)
    return res

"""
# Obtiene los valores mínimo, target y máximo del catálogo para cada concepto que se graficará que inicia con aaaaa
"""
def get_values_conceptos_to_graph( prod_catalogo, conceptos_to_graph ):
...
"""
# Obtiene los registros de Inspección con los datos que vamos a graficar
"""
# def get_records_data( form_id, filter_producto, filter_proceso, list_ids_to_graph=[] ):
#     query = {
#         'form_id': form_id,
#         'deleted_at': {'$exists': False}
#     }
#     query.update(filter_producto)
#     query.update(filter_proceso)
#     select_columns = {'folio': 1, 'answers': 1}
#     records_found = cr.find(query, select_columns)
#     datos = { l: [] for l in list_ids_to_graph }
#     for rec in records_found:
#         for d in datos:
#             datos[ d ].append( rec.get('answers', {}).get(d, 0) )
#     return datos
...
def get_production_qty(form_id, date_from, date_to, product_serach, orden_produccion=None):
    ...
    {'$project':{
        '_id':0,
        'producto':'$answers.60c238ac1d17c02907171593.00000ffff717042907171594',
        'qty':{'$sum':[
            '$answers.60ccc2fda915e60a9e175560',
            '$answers.60cbcf6092841ffc25ca864a',
            '$answers.60ccc5173785a8b10c17553d',
            '$answers.60ccc70a58195a4d5beb1188',
            '$answers.aaaaaf6092841ffc25ca863d']},
        'falla':'$answers.6104630669843e373836fcbc',
        'fecha':'$end_date'
    }},
    {'$group':{
      '_id':{
    ...
    group_rec = cr.aggregate(aggregate_query)
    falla = ""
    qty=0
    for g in group_rec:
        #print('g={}'.format(g))
    ...
    res ={}
    if falla or qty:
        res.update({'a00000000000000000000005':qty, 'a00000000000000000000006':falla})
    #todo acomdar las fallas, quitar las fechas que no tengan falla y ponerlas en text
    return res

def get_data(form_id, filter_producto, filter_proceso, list_ids_to_graph=[], orden_produccion=None, codigo_producto=None,
    ...
    match_query.update(get_date_query(fecha_inicio, fecha_fin))
    ...
    records = cr.aggregate(aggregate_query)
    ...
    for a in records:
        #print('a={}'.format(a))
        for d in datos:
            datos[ d ].append( format_data(a, d) if format_data(a, d) !=None else None )
    return datos

def format_data(row, parameter):
    res = None
    if row.get(parameter):
        try:
            ...
            val_to_param = 0
        res = {
                'd000000000000000000000b1': row.get('fecha'),
                'd000000000000000000000b2': val_to_param,
                'd000000000000000000000b3': row.get('auditor'),
            }
    return res

def proceso_graficacion(current_record, record_id):
    answers = current_record.get('answers', {})

    fecha_inicio = answers.get('f00000000000000000000001', '')
    fecha_fin = answers.get('f00000000000000000000002', '')
    ...
    if not field_id_to_graphs:
        ...
        set_status_proceso(current_record, record_id, 'error', msg='No hay campo grupo repetitivo donde el id empiece por fffff para depositar las gráficas generadas')
        return False
    if not field_forma_inspeccion:
        print('No hay campo de forma de inspección')
        set_status_proceso(current_record, record_id, 'error', msg='No hay campo de forma de inspección')
        return False

    # Obtengo el label de los campos, después me servirá para los nombres de cada gráfico a generar
    form_id_inspeccion = answers.get( field_forma_inspeccion, 0 )
    form_fields = lkf_api.get_form_id_fields(form_id_inspeccion, jwt_settings_key='USER_JWT_KEY')
    fields = form_fields[0]['fields']
            ...
            for i in ans:
                if re.match(r"^0000[0|1|2]ffff(\d{5})", i):
                    ...
                    try:
                        ...
                        set_status_proceso(current_record, record_id, 'error', msg='Campo mal configurado en el id del catalogo {}'.format( id_catalogo_campo ))
                        return False
                    if tipo_campo == '00000':
                        producto = ans[i]
    ...
    prodcut_detail = get_production_qty(form_id_inspeccion, fecha_inicio, fecha_fin, product_serach)
    print('prodcut_detail',prodcut_detail)
    current_record['answers'].update(prodcut_detail)
    prod_catalogo = get_record_producto_catalogo( id_catalogo_producto, field_id_catalogo_producto, producto )
    if prod_catalogo:
        list_graficos = []
        list_datos = []
        # print('registro de producto encontrado')
        #print('prod_catalogo=',prod_catalogo)
        values_to_graph = get_values_conceptos_to_graph(prod_catalogo, conceptos_to_graph)
        # print('===== values_to_graph=',values_to_graph)
        data_records = get_data( form_id_inspeccion, dict_producto, dict_proceso_maquina, list_ids_to_graph=conceptos_to_graph, fecha_inicio=fecha_inicio, fecha_fin=fecha_fin )
        ...
        for val_graph in values_to_graph:
            ...
            data = [ s.get('d000000000000000000000b2', 0) for s in data_records.get( val_graph, [] ) if s]
            if data:
                # try:
                ...
                #     continue
                set_graph = lkf_chart.get_graph_info(values_min_max, data, current_record['form_id'])
                #print('===== grafico generado=',set_graph)
                list_graficos.append( set_graph )
            for s in data_records.get( val_graph, [] ):
                if s:
                    s.update({'d10000000000000000000001': label_item})
                    list_datos.append( s )
        if list_graficos:
            current_record['answers'].update({
                field_id_to_graphs: list_graficos,
                'd000000000000000000000b0': list_datos,
                'a00000000000000000000003': 'realizado',
                'a00000000000000000000004': ''
                })
            lkf_api.patch_record(current_record, record_id, jwt_settings_key='USER_JWT_KEY')
        else:
            set_status_proceso(current_record, record_id, 'error', msg='No se encontró información')
...
if __name__ == "__main__":
    ...
    lkf_chart = Lkf_Chart()
    set_status_proceso(current_record, record_id, 'procesando')
    proceso_graficacion( current_record, record_id )
